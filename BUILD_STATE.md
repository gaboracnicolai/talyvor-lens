...
> **Mint chokepoint count:** `mintTypeList` (`internal/mining/mint_gate.go`) is now **10** entries — the original 7 (`cache_mine · compute_mine · embedding_mine · annotation_mine · pattern_mine · receipt_mine_provisional · pool_royalty_held`) + **`eval_contribution_held`** (#250) + **`eval_routing_prediction_held`** (#260) + **`eval_latency_locality_held`** (#265). `TestMintTypes_GateSet` pins all 10; `TestMintTypeList_IsSingleSource` pins `len(mintTypes)==len(mintTypeList)`. The U6 floor + 1000-LENS/24h rate cap cover every entry. None of the three P-o-I mints is in `isReputationBondedType` (= `{receipt_mine_provisional, pool_royalty_held}`) — the #9 bond no-ops for all three (decision c, symmetric).
...
| `HeldBenchmarkAnchor{rate}` = `rate × clamp01(HeldScore)` (the #10 held-ground-truth pattern) — **rate REQUIRED** (`NewHeldBenchmarkAnchor` rejects 0/neg/NaN/Inf, no default mint) | BUILT · now **THREE live callers** (§A18/§A23/§A26) | `anchor.go` `NewHeldBenchmarkAnchor`; reachability AST guard `anchor_test.go` `TestHeldBenchmarkAnchor_ExactlyThreeLiveCallers` — sanctioned set `{eval_contribution_minter.go, routing_prediction_minter.go, latency_minter.go}`; any other non-test `NewHeldBenchmarkAnchor`/`SetAnchor` caller fails it |
...
| Component | Status | file:line |
|---|---|---|
| `HeldBenchmarkAnchor` first live caller | BUILT-INERT | `internal/poolroyalty/eval_contribution_minter.go` `NewEvalContributionMinter` (the FIRST of three non-test `NewHeldBenchmarkAnchor` callers — the guard is now `anchor_test.go` `TestHeldBenchmarkAnchor_ExactlyThreeLiveCallers`) |
...
> **P-o-I piece-3 arc — COMPLETE:** PR-1 unit (§A19, #252) · PR-2 cohort tags (§A20, #253) · PR-3a scorer (§A21, #254) · forward oracle (#255) · PR-3b A′ extraction (§A22, #256) · applyKey oracle (#258) · PR-3c full type move + real Inferer (§A22, #259) · **PR-4 the mint (§A23, #260)** — all DONE. **The Proof-of-Improvement primitive now has THREE mints** — eval-contribution (§A18, #250) + routing-prediction (§A23, #260) + latency-locality (§A26, #265) — **all BUILT-INERT, all unbonded**, all routed through the U6 chokepoint, all gated behind a force-off flag + a default-0 rate. The held-benchmark anchor has exactly **three** live callers.
...
### A26 · Proof-of-latency-locality — the LATENCY HALF of P3 #6 (P-o-I instance 3) — PRs #263 / #264 / #265
**BUILT-INERT · the LATENCY half of #6 is COMPLETE; the LOCALITY half is DEFERRED (§C7).** A node is paid for genuinely-fast service — cohort-relative + cost-weighted + quality-gated — off a descriptive latency capture, through the SAME held-ledger / U6 chokepoint. The **THIRD** live `HeldBenchmarkAnchor` caller, and the **FIRST epoch-settled** mint (instances 1/2 are per-event/`request_id`-keyed). Three PRs:

| Layer | Status | file:line / migration |
|---|---|---|
| **L1+L2 · descriptive capture** (#263) | BUILT-INERT | `internal/nodelatency/store.go` `RecordServe` (Exec-only, no ledger) + `internal/proxy/nodelatency_capture.go` `captureNodeLatency` (obsLimiter-bounded post-serve, mirrors worktier); times the node round-trip in `tryNodeRouting`, derives cohort (`cohort.DeriveInputCohort`) + cost (`router.AnalyseComplexity`). Rolling **α=0.2 EWMA** into **0074** `node_cohort_latency_stats`. Flag `LENS_NODE_LATENCY_CAPTURE_ENABLED` **false**, **NOT** force-off (descriptive). |
| **L3.5 · model grain** (#264) | BUILT-INERT | **0075** DROP+re-CREATEs `node_cohort_latency_stats` with the **5-col PK** `(node_id, feature_category, input_token_range, complexity_bucket, model)` (with a non-empty guard that atomically rolls back under the transactional runner) — so the C-gate aligns exactly to `benchmark_node_scores`' `(node, model)` grain. |
| **L4 · the MINT** (#265) | BUILT-INERT | `internal/poolroyalty/latency_minter.go` (3rd anchor caller) + **0076** `node_latency_mints` (generic finalize columns ⇒ generic `FinalizeSweeper` settles it). |

- **Composed pay:** `latency_skill = C_passed ? clamp01((baseline − latency_ewma)/baseline) × (cohortCost/5) : 0`; `amount = anchor.Value(HeldScore: latency_skill)` = `rate × clamp01(latency_skill)`. `cohortCost = cost_weight_accum/sample_count` (avg `AnalyseComplexity` score; max 5).
- **Anti-farm (all in `latencyScanSQL`):** the baseline is a **PER-CANDIDATE median** of `latency_ewma` over nodes **fingerprint-UNLINKED to that candidate** (mirrors `eval_contribution_minter.go`'s `evalDiscriminationSQL` workspace-exclusion — excludes self AND the candidate's `workspace_card_fingerprints`-linked set); a cohort needs **≥ 3 distinct unlinked peers** (`MinUnlinkedNodes`) or pays nothing. The quality gate is **EXACT per-(node,model)**: JOIN `benchmark_node_scores` ON `(node_id, model)`, `score ≥ 0.7 AND sample_count ≥ 5` (node-blind held probes ⇒ fast-and-wrong closed). Payee = `node_id → inference_nodes.workspace_id` (INNER JOIN, orphan drops). **Batch cap 500** candidates/tick (money-safety; the per-candidate exclusion is correlated).
- **Settlement:** `epoch = floor(now/86400)` (24h); `request_id = SHA256Hex(node:feature:itr:complexity:model:epoch)` UNIQUE ⇒ `ON CONFLICT DO NOTHING` exactly-once-per-window; a fresh epoch re-pays sustained speed. **FINAL-EWMA snapshot** — RunOnce reads the *current* `latency_ewma`, so a boundary front-load burst can't lock a fast rate then serve slow.
- **NO-LOOP** (`latency_noloop_test.go`): reads `node_cohort_latency_stats` ⋈ `inference_nodes` ⋈ `benchmark_node_scores` ⋈ `workspace_card_fingerprints` by RAW SQL; writes only `node_latency_mints` + the ledger; imports none of `nodelatency`/`benchprobe`/`routingscore`/`proxy`.
- **New mint type:** `TypeLatencyLocalityHeld = "eval_latency_locality_held"` → `mintTypeList` (**9→10**). NOT reputation-bonded (decision c). Earning flag `LENS_LATENCY_MINTING_ENABLED` **false**, **IN the force-off block**; manifest **13→14**. Rate `LENS_LATENCY_RATE_PER_POINT` **0**. Reachability guard **"two"→"three"**.
- **DOUBLE-INERT:** rate-0 ⇒ nil anchor ⇒ `RunOnce` no-op **AND** the flag default-off — mints ZERO by default even with `mintTypeList` at 10. real-PG `-race`: mint-correctness asserts the **exact** amount (4.00), exactly-once + next-epoch re-pay, final-EWMA (100→50 ⇒ 4.00→5.00), C-gate per-(node,model), min-nodes, baseline-excludes-linked-sockpuppets, batch-cap bound, U6 rollback (`internal/poolroyalty/latency_minter_integration_test.go`).
...
1. **Force-off set is 14** (verified `economy_killswitch_test.go` `len(checks) == 14`). The 14 are the original 11 (pattern trio, pool-royalty, povi, trustful-compute, cache-sharing/poolable, distill-poolable, routing-intelligence, tier-cohorts) + the three P-o-I EARNING gates `EvalContributionMintingEnabled` (#250) + `RoutingPredictionMintingEnabled` (#260) + `LatencyMintingEnabled` (#265) — all MINT. The capability/measurement/descriptive flags (`NodeAutoRoute`, `ReputationBondedMinting`, `ProofOfBenchmark`, `ProofOfImprovement`, `RoutingPrediction`, `RoutingPredictionScoring`, `NodeLatencyCapture`) are DELIBERATELY excluded — each routes / reduces / suppresses / measures / describes, never creates a mint.
2. **`mintTypeList` is 10** (verified — `mint_gate.go` + `TestMintTypes_GateSet`): the original 7 + `eval_contribution_held` (#250) + `eval_routing_prediction_held` (#260) + `eval_latency_locality_held` (#265). All three P-o-I mints are **absent from `isReputationBondedType`** (= `{receipt_mine_provisional, pool_royalty_held}`) — decision c, the #9 bond no-ops for all three.
...
5. **config.go line citations: §A1–§A17 + the §B numeric knobs are CARRIED FORWARD from the `c4742ab` regen and NOT re-verified this pass** (config.go has grown by ~hundreds of lines since; older `:NNN` cites are approximate; the subsystems are unchanged). The structural counts — manifest **14**, `mintTypeList` **10**, latest migration **0076**, **HeldBenchmarkAnchor live callers = 3** (`eval_contribution_minter.go` + `routing_prediction_minter.go` + `latency_minter.go`; `anchor.go` is the definition) — are VERIFIED at `7f2ebd2` (read from the tree this pass), not transcribed.
...
7. **The phase axes do not correspond — the wishlist axis is FOUNDER-DEFINED, not repo-tracked.** The founder's wishlist token-economy grouping (P1 Foundation / P2 Intelligence / P3 …, over items #1–#10) is a DIFFERENT numbering from the repo's own phases and **is not enumerated in the repo**. Do not conflate it with: the repo's `ROADMAP.md` "Phase 3 — deepen the mineable primitives" (`:44`, a DIFFERENT axis, **stale** — dated `5d52ea7`, pre-P-o-I-arc); COORDINATION.md's "Phase 3 = routing-pattern earning" (`:77`, done); or "PoVI Phases 1–5" (COORDINATION.md:14). **Wishlist axis status:** P1 (Foundation) done · P2 (Intelligence) build-complete-except-#5 (§A25) · **P3 (founder-defined) = #6 proof-of-latency-locality, #7 carbon-verified green mining, F2 idle-mind network** — status:
>    - **#6 — LATENCY HALF COMPLETE (§A26, #263/#264/#265), LOCALITY HALF DEFERRED.** The latency mint is built-inert (cohort-relative, cost-weighted, per-(node,model) quality-gated, epoch-settled). The **locality** half is **DEFERRED — no substrate**: there is no requester-origin capture and no gateway-VERIFIED node position; both are node-declarable (trivially spoofable if node-asserted), so a locality mint is not buildable until a trustworthy geo/position substrate exists (a gateway-measured signal, not a node claim). Same "verified-substrate-first" pattern as #5 / the locality half.
>    - **#7 carbon-verified green mining + F2 idle-mind network — NOT RECONNED.** Both are flagged likely to hinge on a **"verified" substrate that may not exist** (carbon attestation for #7; verified-idle + opt-in + embeddings-only isolation for F2) — the same gate that blocked #5 and #6-locality. Recon each before assuming buildable.

### §D6 — money-path identity hardening (branch `lens-moneypath-identity-hardening`)

_Appended at `92e8b56`; the manifest body above is still derived from `7f2ebd2`. Defense-in-depth only — no ledger arithmetic, supply, mint amount, or CEIL/FLOOR rounding was touched._

- **Property A — SEC-7 spend-alert replay/freshness: ALREADY CORRECT (no change).** The Lens→Track emitter `internal/alerts/emitter.go` already stamps a server-owned `event_id` (`uuid.NewString()`, unique per emit — `:87`) and `emitted_at` (server clock, RFC3339 — `:88`), added by PR #293 (`bf0cca3`, 2026-07-13) and covered by `emitter_test.go` (event_id set / unique-per-emit / freshness). The durable dedup + rejection RECEIVER lives in **Track** (`lensintegration`), not this repo. It is a JSON wire field — **no DB column, no migration**. The "`SpendAlertPayload` has no event_id" note was stale.
- **Property B — server-owned identity on money-path writes: BUILT (commit `37060f4`).** Two write primitives that keyed their `UPDATE` on `id` ALONE now key on `(id, workspace_id)`, so a wrong/foreign id can never touch another workspace's row (the SEC-11 "the scoping key holds every identity it protects" lesson):
  - `budgets.Store.UpdateSpent(ctx, workspaceID, id, spent)` — `WHERE id=$1 AND workspace_id=$3`. `spent_usd` is Tier-3 ADVISORY USD (not µLENS); value written unchanged, only row scoping added. Caller threads server-loaded `b.WorkspaceID`.
  - `povi.ChallengeStore.UpdateResult(ctx, workspaceID, id, result, slashedAmount, reason)` — `WHERE id=$1 AND workspace_id=$5`. `slashedAmount` (µLENS, SEC-2) written UNCHANGED. Caller threads `ch.WorkspaceID`; interface `challengeStore` + both test fakes (`memChallengeStore`, `noopChallengeStore`) updated to the new signature (never deleted).
  - `tenant.Store.ValidateAPIKey` was named by the property but is ALREADY CORRECT — it derives the workspace from the authenticated key's own DB row (`k.WorkspaceID`), never a client header. Left untouched.
  - Proofs: real-PG two-workspace confinement tests; **mutation-verified** (reverting each `WHERE` to id-only makes the cross-workspace write LEAK → test fails). **No migration** — both tables already carry `workspace_id` (budgets `0028`, povi_challenges `0033`; high-water `0096` unchanged). Full from-zero real-PG CI green (`-race -p 1`, 87 ok, 0 skip).
...
> **Pre-public-mint GO-LIVE gates (all three P-o-I mints stay INERT until ALL clear):** the gateway-bound `request_id` receipt-trust gate (above) + the different-card sockpuppet bound + audit + legal + live Stripe + the deliberate flip (lift the force-off flag AND set a positive rate). The latency mint (§A26) additionally depends on the **proof-of-benchmark scheduler running** (to populate `benchmark_node_scores` for its C-gate) and on a **live node fleet** (its cohort-relative baseline is meaningful only with ≥3 unlinked nodes per cohort — a density/prematurity note, not a code gap). Merging the mint PRs (#250, #260, #265) built the mints; it armed nothing.
