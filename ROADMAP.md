...
- **DISTILL attribution — DONE on main (S1, PR #148, 41f9455).** Durable, mint-free per-(owner, requester, artifact) counter of consented cross-tenant pooled-distill serves (`distill_serve_attribution`, migration 0052). Write-only — no read surface yet (read surface committed: admin-only raw rows, counterparty masked as counts, content_hash never cross-tenant).
