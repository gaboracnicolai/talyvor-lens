...
- **Routing-pattern EARNING anti-gaming controls — all THREE built INERT + CI-proven:** rarity bound (S1, PR #116: feature excluded from the rarity key + a cross-workspace corroboration floor), per-window earn cap (S2, PR #117: concurrency-exact single-tx, real-defaulted 50k/24h), idempotency claim (S3, PR #118: migration 0049 pattern_mine_credits, composite UNIQUE(request_id, workspace_id)). RecordPattern stays unwired — these are built ahead of the wire-up, the same bar Pool-B minting required.
- **Remaining: S4 — the gated wire-up** (the only stage that flips earning live, behind LENS_PATTERN_EARNING_ENABLED). It carries HARD flip-on preconditions: (a) request_id serve-bound (server-minted/serve-hash), NOT the raw caller header; (b) workspace_id authenticated (rides #84), not a caller-asserted header; (c) serve-path error belt (earn errors swallowed, detached ctx); (d) Sybil controls on opted-in-workspace creation. Full detail in COORDINATION.md "Routing-pattern EARNING gate" + the CONSOLIDATED FLIP-ON CHECKLIST.
