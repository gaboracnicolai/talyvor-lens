...
// 0099_token_events_serve_source) into a private schema and returns a pool pinned to it — the
...
// The serve_source vocabulary is enforced in the schema (0099 CHECK) so a typo'd layer label can
...
func TestRecordCacheServe_RealPG_RejectsUnknownServeSource(t *testing.T) {
	...
	if err == nil {
		t.Fatal("RecordCacheServe accepted serve_source=cache_hit_bogus — the 0099 CHECK constraint must reject it")
	}
}

// MONEY-DIFF DISCIPLINE: the upstream write path must be byte-unchanged by 0099. A RecordSpend row
