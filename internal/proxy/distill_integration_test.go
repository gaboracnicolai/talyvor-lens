...
func TestMaybeDistill_AnthropicDoc_OptIn(t *testing.T) {
	...
	newBody, newPrompt, newMod, did, _, _ := d.MaybeDistill(context.Background(), reqWith("true"), body, "ws1", modality.Detect(body), nil)
	if !did {
...
func TestMaybeDistill_NotOptedIn_Inert(t *testing.T) {
	...
	newBody, _, _, did, _, _ := d.MaybeDistill(context.Background(), reqWith(""), body, "ws1", modality.Detect(body), nil)
	if did {
...
func TestMaybeDistill_PolicyDisabled_Inert(t *testing.T) {
	...
	body := anthropicDocBody(t, false)
	newBody, _, _, did, _, _ := d.MaybeDistill(context.Background(), reqWith("true"), body, "ws1", modality.Detect(body), nil)
...
func TestMaybeDistill_Always_NoHeader(t *testing.T) {
	...
	d := newDistiller(t, conv, workspace.DistillAlways)
	body := anthropicDocBody(t, false)
	_, _, _, did, _, _ := d.MaybeDistill(context.Background(), reqWith(""), body, "ws1", modality.Detect(body), nil)
	if !did {
...
func TestMaybeDistill_NeedsVision_VisionFailureGraceful(t *testing.T) {
	...
	body := anthropicDocBody(t, false)
	newBody, _, _, did, _, _ := d.MaybeDistill(context.Background(), reqWith("true"), body, "ws1", modality.Detect(body), vis)
	if did {
...
func TestMaybeDistill_OpenAIDataURI(t *testing.T) {
	...
	_, newPrompt, _, did, _, _ := d.MaybeDistill(context.Background(), reqWith("true"), body, "ws1", modality.Detect(body), nil)
	if !did {
...
func TestMaybeDistill_NilDeps_Inert(t *testing.T) {
	...
	for _, d := range []*distillIntegration{
		...
		nb, _, _, did, _, _ := d.MaybeDistill(context.Background(), reqWith("true"), body, "ws1", modality.Detect(body), nil)
