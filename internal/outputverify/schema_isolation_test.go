...
// schema (search_path = lens_it_outputverify, public) — the peer harness 16 other packages ship. Without
...
// Test-only: it changes no production code. `public` stays in the path so the shared vector extension
// resolves; tables are created in the private schema (first in the path). No-op when the env var is unset.
	...
	if base := os.Getenv("LENS_TEST_DATABASE_URL"); base != "" {
		...
		if admin, err := pgxpool.New(ctx, base); err == nil {
			if tx, terr := admin.Begin(ctx); terr == nil {
				...
				_, _ = tx.Exec(ctx, "DROP SCHEMA IF EXISTS lens_it_outputverify CASCADE")
				_, _ = tx.Exec(ctx, "CREATE SCHEMA lens_it_outputverify")
				_ = tx.Commit(ctx)
			}
			admin.Close()
			sep := "?"
			...
			os.Setenv("LENS_TEST_DATABASE_URL", base+sep+"options="+url.QueryEscape("-c search_path=lens_it_outputverify,public"))
		}
	}
	os.Exit(m.Run())
}
