...
func TestAgentSpend_SyntheticMidTxCrash_NoOrphanClaim(t *testing.T) {
	...
	err := sf.SpendLXCForAgent(ctx, "keyX", "wsX", "req-crash", 10, "t")
	if err == nil {
	...
	if bal != 100 || spent != 0 || claims != 0 {
		t.Fatalf("mid-tx crash must roll back EVERYTHING: bal=%.2f spent=%.2f claims=%d, want 100/0/0", bal, spent, claims)
	}
	var claimRow int
	...
	if err := s.SpendLXCForAgent(ctx, "keyX", "wsX", "req-crash", 10, "t"); err != nil {
		t.Fatalf("retry of the SAME request_id after a rolled-back crash must SUCCEED (id not burned), got %v", err)
	}
	if bal, spent, claims := agentState(t, s, "wsX", "keyX"); bal != 90 || spent != 10 || claims != 1 {
		t.Fatalf("post-crash retry must debit EXACTLY once: bal=%.2f spent=%.2f claims=%d, want 90/10/1", bal, spent, claims)
	}
}
