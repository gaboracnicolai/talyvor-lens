...
// schema (search_path = lens_it_economy, public). Without it, the parallel `go test ./...` package binaries
	...
	if base := os.Getenv("LENS_TEST_DATABASE_URL"); base != "" {
		...
		if admin, err := pgxpool.New(ctx, base); err == nil {
			...
			if tx, terr := admin.Begin(ctx); terr == nil {
				...
				_, _ = tx.Exec(ctx, "DROP SCHEMA IF EXISTS lens_it_economy CASCADE")
				_, _ = tx.Exec(ctx, "CREATE SCHEMA lens_it_economy")
				_ = tx.Commit(ctx)
			}
			admin.Close()
			sep := "?"
			...
			os.Setenv("LENS_TEST_DATABASE_URL", base+sep+"options="+url.QueryEscape("-c search_path=lens_it_economy,public"))
		}
	}
	os.Exit(m.Run())
}
