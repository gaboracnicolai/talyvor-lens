...
func seedAvailable(t *testing.T, pool *pgxpool.Pool, ws string, amount float64) {
	t.Helper()
...
func readBalance(t *testing.T, pool *pgxpool.Pool, ws string) (available, locked float64) {
	t.Helper()
	err := pool.QueryRow(context.Background(),
...
func ledgerBurned(t *testing.T, pool *pgxpool.Pool, ws string) float64 {
	t.Helper()
	var burned float64
	err := pool.QueryRow(context.Background(),
...
func approxEq(a, b float64) bool { return math.Abs(a-b) <= concEps }
...
type slashRecon struct {
	amount     float64 // povi_stakes.amount
	slashedRec float64 // povi_stakes.slashed_amount (audit)
	locked     float64 // lens_token_balances.locked_balance (ledger truth)
	burnLedger float64 // Σ slash deltas in the ledger (real burn)
	sumReturns float64 // Σ successful StakeManager.Slash() returns
	okCount    int
}
...
func runConcurrentSlash(t *testing.T, pool *pgxpool.Pool, node string, live bool, n int, f, S float64) slashRecon {
	t.Helper()
	ctx := context.Background()
	ws := wsFor(node)
...
func TestPoVIStakeSlashConcurrency_RealPG(t *testing.T) {
	...
	t.Run("GREEN_advisory_lock_holds_invariants", func(t *testing.T) {
		...
		t.Logf("GREEN: amount=%.6f locked=%.6f slashed_rec=%.6f burn_ledger=%.6f Σreturns=%.6f ok=%d/%d",
			r.amount, r.locked, r.slashedRec, r.burnLedger, r.sumReturns, r.okCount, N)
		if r.amount < -concEps {
			t.Fatalf("invariant: amount must never go negative; got %.6f", r.amount)
		}
		if !approxEq(r.amount, r.locked) {
			t.Fatalf("invariant 2 (locked_balance integrity): povi_stakes.amount=%.6f must equal ledger locked_balance=%.6f", r.amount, r.locked)
		}
		if !approxEq(r.slashedRec, r.burnLedger) {
			t.Fatalf("invariant 1 (audit): slashed_amount=%.6f must equal real burn=%.6f", r.slashedRec, r.burnLedger)
		}
		if !approxEq(r.sumReturns, r.burnLedger) {
			t.Fatalf("invariant 6 (supply): Σ Slash() returns=%.6f must equal ledger burn=%.6f", r.sumReturns, r.burnLedger)
		}
		if !approxEq(r.locked+r.burnLedger, S) {
			t.Fatalf("invariant 1 (conservation): remaining+burned=%.6f must equal seeded stake %.6f", r.locked+r.burnLedger, S)
		}
		if r.okCount != N {
	...
	t.Run("RED_bypassed_lock_corrupts", func(t *testing.T) {
		...
		for attempt := 1; attempt <= maxAttempts && !demonstrated; attempt++ {
			...
			if recordVsLedger || auditVsBurn {
				...
				t.Logf("RED attempt %d CORRUPTED (as expected without the lock): "+
					"amount=%.6f vs ledger locked=%.6f (Δ=%.6f); slashed_amount(audit)=%.6f vs real burn=%.6f (Δ=%.6f). "+
		...
		if !demonstrated {
			t.Fatalf("the lock-bypassed (db=nil) path stayed consistent across %d attempts "+
				"(last: amount=%.6f locked=%.6f slashed_rec=%.6f burn=%.6f) — the advisory lock may "+
	...
	t.Run("GREEN_anti_yank_slash_vs_release", func(t *testing.T) {
		...
		t.Logf("anti-yank: status=%s amount=%.6f locked=%.6f burned=%.6f returned=%.6f slashErr=%v relErr=%v",
			st.status, st.amount, locked, burned, returned, slashErr, relErr)

		if !approxEq(locked, 0) {
			t.Fatalf("collateral must fully leave the locked state exactly once; locked=%.6f", locked)
		}
		if !approxEq(burned+returned, A) {
			t.Fatalf("anti-yank: burned(%.6f)+returned(%.6f) must equal the collateral %.6f — no double-spend", burned, returned, A)
		}
		switch st.status {
		case string(povi.StakeSlashed):
			if !approxEq(burned, A) || !approxEq(returned, 0) {
				t.Fatalf("slash won: expect burned≈%.6f returned≈0; got burned=%.6f returned=%.6f", A, burned, returned)
			}
			if relErr == nil {
		...
		case string(povi.StakeReleased):
			if !approxEq(returned, A) || !approxEq(burned, 0) {
				t.Fatalf("release won: expect returned≈%.6f burned≈0; got returned=%.6f burned=%.6f", A, returned, burned)
			}
			if slashErr == nil {
	...
	t.Run("GREEN_stake_while_slashing", func(t *testing.T) {
		...
		avail, locked := readBalance(t, pool, ws)
		burned := ledgerBurned(t, pool, ws)
		t.Logf("stake-while-slashing: amount=%.6f locked=%.6f avail=%.6f burned=%.6f Σtopups=%.6f",
			st.amount, locked, avail, burned, sumTopups)

		if !approxEq(st.amount, locked) {
			t.Fatalf("record/ledger desync: amount=%.6f locked=%.6f", st.amount, locked)
		}
		if !approxEq(avail, seed-S-sumTopups) {
			t.Fatalf("lost update on available: got %.6f want %.6f (seed-S-Σtopups)", avail, seed-S-sumTopups)
		}
		if !approxEq(avail+locked+burned, seed) {
			t.Fatalf("conservation: available+locked+burned=%.6f must equal seeded %.6f", avail+locked+burned, seed)
		}
	})
	...
	t.Run("GREEN_terminal_state_no_resurrection", func(t *testing.T) {
		...
		if before.status != string(povi.StakeSlashed) || !approxEq(before.amount, 0) {
			t.Fatalf("setup: expected fully-slashed terminal stake; got status=%s amount=%.6f", before.status, before.amount)
		}
		...
		if after.status != string(povi.StakeSlashed) || !approxEq(after.amount, 0) || !approxEq(locked, 0) {
			t.Fatalf("terminal stake mutated: status=%s amount=%.6f locked=%.6f", after.status, after.amount, locked)
		}
	})
}
