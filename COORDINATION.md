...
- **Phase-2 Stage 2.0 — shared-cache governance gate: EXACT CACHE DONE; semantic deferred to 2.0b.** Shipped the cross-tenant opt-in for the EXACT response cache: global switch `LENS_CACHE_POOLABLE_ENABLED` (default off) + per-workspace `cache_poolable` (default false, migration 0041) + admin `PUT /v1/workspaces/{wsID}/cache-poolable`; a read-only `internal/cache_pooling` gate; pooled writes/reads under a separate un-prefixed key tagged with the contributor (exact cache's existing `SetWithOwner`/`GetWithOwner`). A pooled hit requires ALL of global+requester+contributor opted-in; private-by-default and byte-for-byte inert when off; PII never pooled; **zero ledger writes; #84 isolation internals untouched (additive — reads the resolved wsID).** DEFERRED to **Stage 2.0b**: the semantic/pgvector cache pooling (needs a provenance migration — `contributor_workspace_id`/`is_poolable` on `prompt_embeddings`, ~0042 — and cross-tenant similarity matching). Still the precondition for Pool-B royalty minting (the ledger seam, stays gated on seam #1). Highest-leverage non-build move remains a first pilot customer.
...
- **Phase-2 Stage 2.0 — shared-cache governance gate: EXACT CACHE DONE (semantic → 2.0b).** Exact-cache cross-tenant pooling shipped: opt-in (global `LENS_CACHE_POOLABLE_ENABLED` + per-workspace `cache_poolable`), read-only `internal/cache_pooling` gate, pooled writes/reads via the exact cache's existing owner provenance, all inert by default. Additive on top of #84 (reads the resolved wsID; isolation internals untouched). New migration **0041** (workspaces.cache_poolable) follows seam #2 (additive, own file). **No ledger writes → does NOT touch seam #1.** Semantic/pgvector pooling deferred to Stage 2.0b (needs a ~0042 provenance migration on prompt_embeddings).
- **🔔 HEADS-UP TO COLLABORATOR (Andrei):** the cache layer is now in motion — new migration **0041** adds `workspaces.cache_poolable`, and an opt-in cross-tenant exact-cache pooling path landed (additive, default-off, #84 isolation untouched). Pool-B royalty **minting stays gated on seam #1** (no ledger code in this work). Sync before touching the cache layer or `prompt_embeddings` (the 2.0b provenance migration).
