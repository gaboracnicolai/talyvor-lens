...
- **🔔 2026-06-07 — Coordination item for Andrew (his CI):** the Pool-B cap's exactness proof (`TestCapExactness_ConcurrentSamePair_Integration`) and YOUR dbmigrate integration tests are both `LENS_TEST_DATABASE_URL`-gated and currently SKIP in CI (no postgres service in ci.yaml). Adding a `services: postgres` block + exporting the var would give both standing regression guards. Your workflow file — sync before we touch it.
